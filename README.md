...
| 开发机 Windows | 双击 `启动真机.bat` | 本机 Python 3.10；若目录内已有 `runtime\python310` 则优先用私有运行时 |
...
1. 打开后直接在机械臂页。
2. 本机即可动臂。急停保持当前姿态（电机不停电），不是卸力砸臂；急停后仍可点「复位」受控回零。其它运动需先解除急停。
...
4. Gravity：仿真拖滑条示教；真机为力矩环（有 pinocchio 走 SDK，否则站控 `G(q)` + MIT）。Position 位置控制不变。连续示教默认 **拖臂录制**（开始后切入 Gra+Fri/Gravity），也可切 **软件录制**；回放先慢速走到起点再按时间戳 `servo_j`。可选中文件删除。路点面板仍是稀疏关键帧，由后端插补，可设每点时长。脚本只映射到本站 API，不另起进程抢串口。
...
规格：[`docs/SPEC.md`](docs/SPEC.md) · 使用说明：[`docs/使用说明.md`](docs/使用说明.md) · 打包：[`packaging/windows/README.md`](packaging/windows/README.md)
