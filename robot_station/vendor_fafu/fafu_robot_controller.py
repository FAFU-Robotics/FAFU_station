...
class FafuRobotController:
    ...
    def _precheck_communication(self) -> None:
        """Require joint motors on the bus; gripper timeout is a warning.

        A missing joint cannot be commanded safely. The gripper is an
        independent actuator, so a silent or faulted M7 must not lock the
        user out of the menu / joint motion.
        """
        ...
        if bad_joints:
            raise RuntimeError(
                f"motors {bad_joints} did not respond within 500ms; "
