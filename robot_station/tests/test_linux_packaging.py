...
class LinuxPackagingFilesTests(unittest.TestCase):
    def test_packaging_kit_exists(self) -> None:
        ...
        self.assertTrue((PACK / "handtest_open_window.sh").is_file())
    ...
    def test_build_refuses_customer_image_without_sdk(self) -> None:
        ...
        probe = (PACK / "gtk_probe.c").read_text(encoding="utf-8")
        self.assertIn("webkit2/webkit2.h", probe)
        self.assertIn("webkit_get_major_version", probe)
        ...
        self.assertIn("webkit2gtk-4.0", script)
        self.assertIn("linuxdeploy did not bundle libwebkit2gtk", script)
        self.assertIn("cc, gtk+-3.0 and webkit2gtk-4.0 are required", script)
    ...
    def test_verify_requires_fafu_motor_and_manifest(self) -> None:
        ...
        self.assertIn("libwebkit2gtk-4.0.so", verify)
        self.assertIn("libjavascriptcoregtk-4.0.so", verify)
        self.assertIn("webkit-gi-ok", verify)
        self.assertIn("webkit_present", verify)
        self.assertIn("gi.require_version('WebKit2','4.0')", verify)
    ...
    def test_linux_customer_readme(self) -> None:
        ...
        preflight = (ROOT / "robot_station" / "linux_preflight.py").read_text(encoding="utf-8")
        self.assertIn("probe_fafu_motor", preflight)
        self.assertIn("STATION_PORTABLE", preflight)
        arm = (ROOT / "robot_station" / "adapters" / "fafu_arm.py").read_text(encoding="utf-8")
        self.assertIn("resolve_live_serial_port", arm)
        self.assertIn("fafu_robot_cpp/linux", arm)
        overlay = (ROOT / "robot_station" / "vendor_fafu" / "fafu_robot_controller.py").read_text(
            encoding="utf-8"
        )
        self.assertIn("resolve_live_serial_port", overlay)
        ...
        self.assertIn("handtest_open_window.sh", accept)
        self.assertIn("Live", accept)
        self.assertIn("仿真臂", accept)
        hand = (PACK / "handtest_open_window.sh").read_text(encoding="utf-8")
        self.assertIn("WSL is not ACCEPTANCE B", hand)
        self.assertIn("libwebkit2gtk-4.0.so", hand)
        self.assertIn("webkit-gi-ok", hand)
        self.assertIn("无法打开独立控制窗口", hand)
        self.assertIn("仿真臂", hand)
        self.assertIn("gir1.2-webkit2-4.0", hand)
        self.assertIn("FAFUAPP1", hand)
        usage = (ROOT / "docs" / "使用说明.md").read_text(encoding="utf-8")
        self.assertIn("linux-appimage", usage)
        self.assertIn("Jetson", usage)
        self.assertIn("/dev/fafu_debug_board", usage)
        live_sh = (ROOT / "启动真机.sh").read_text(encoding="utf-8")
        self.assertIn("STATION_ALLOW_LIVE_ARM=1", live_sh)
        alias = ROOT / "start_live_arm.sh"
        self.assertTrue(alias.is_file())
        self.assertIn("启动真机.sh", alias.read_text(encoding="utf-8"))
        example = (ROOT / "configs" / "station.example.yaml").read_text(encoding="utf-8")
        self.assertIn("启动真机.sh", example)
