...
class FafuSimTests(unittest.TestCase):
    ...
    def test_overlay_joint_timeout_does_not_fail_construct(self) -> None:
        from types import SimpleNamespace

        from robot_station.adapters.fafu_arm import _uses_controlled_motors, _with_soft_precheck
        ...
        class OverlayLike:
            ...
            def __init__(self, **_kwargs: object) -> None:
                self._ht = SimpleNamespace(read_motor_state=lambda *_a, **_k: None)
                self._cfg = SimpleNamespace(motor_ids=[1, 2, 3, 4, 5, 6])
                self._gripper_motor_id = 7
                self._joint_motor_ids = [1, 2, 3, 4, 5, 6]
                self._precheck_communication()

            def _precheck_communication(self) -> None:
                raise RuntimeError(
                    "motors [1, 2, 3, 4, 5, 6] did not respond within 500ms; "
                    "check power, wiring and motor IDs"
                )

        self.assertTrue(_uses_controlled_motors(OverlayLike))
        obj = _with_soft_precheck(OverlayLike)()
        self.assertEqual(list(getattr(obj, "_missing_motors", []) or []), [])
    ...
    def test_stream_link_ok_false_when_usb_silent(self) -> None:
        ...
        obj._gripper_motor_id = 7
        obj._missing_motors = [7]
        obj._station_had_rx = True
        self.assertFalse(obj._stream_link_ok())
        self.assertTrue(dead)

    def test_stream_link_ok_when_driver_never_rx(self) -> None:
        from types import SimpleNamespace
        ...
        class Dummy:
            ...
            def _enter_dead(self, reason: str) -> None:
                raise AssertionError(f"never-RX must not latch DEAD: {reason}")
        ...
        obj._missing_motors = []
        self.assertTrue(obj._stream_link_ok())
        self.assertFalse(bool(getattr(obj, "_station_had_rx", False)))
    ...
    def test_sdk_still_defaults_auto_enable_true(self) -> None:
        ...
        overlay = ROOT / "robot_station" / "vendor_fafu" / "fafu_robot_controller.py"
        src_servo = overlay.read_text(encoding="utf-8")
        self.assertIn("frame_ids", src_servo)
        self.assertIn("def servo_start", src_servo)
