...
class TrajFileTests(unittest.TestCase):
    ...
    def test_sanitize_and_rate(self) -> None:
        ...
        self.assertEqual(sanitize_traj_name("抓取A"), "抓取A.jsonl")
    ...
    def test_rename_recording_stays_in_dir(self) -> None:
        with recordings_tmpdir() as tmp:
            path = make_traj_path("oldname")
            TrajectoryWriter(path, teach="soft", mode="Position", n=6).close()
            new = rename_recording(path.name, "抓取演示")
            self.assertEqual(new.name, "抓取演示.jsonl")
            self.assertTrue(new.is_file())
            self.assertFalse(path.is_file())
            listed = {it["name"] for it in list_recordings(tmp)}
            self.assertIn("抓取演示.jsonl", listed)
            self.assertNotIn(path.name, listed)
            other = make_traj_path("taken")
            TrajectoryWriter(other, teach="soft", mode="Position", n=6).close()
            with self.assertRaises(FileExistsError):
                rename_recording(new.name, other.name)
            with self.assertRaises(FileNotFoundError):
                rename_recording(str(tmp / ".." / "outside.jsonl"), "x")
            with self.assertRaises(ValueError):
                rename_recording(new.name, "???")
