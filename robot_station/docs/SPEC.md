...
1. **客户交付（推荐）**：`packaging/windows/build_portable.ps1` 生成 `dist/FAFUArmStation/`。打包时从本机 conda Python 3.10 拷入 **pinocchio**（及 `Library/bin` 原生 DLL；Windows `pip` 装不了真库）。客户解压后双击 `FAFUArmStation.exe` 或 `启动真机.bat`：第一次拷到 `%LOCALAPPDATA%\FAFUArmStation`（不写 PATH），并打开窗口；以后点桌面快捷方式即可。启动真机在 USB 连上后自动 Connect 并使能（SDK 构造仍 `auto_enable=False`）。便携包 Gravity / Gra+Fri 走 SDK `gravity_compensation_step`（`dyn_ready=true`），不得因缺 pinocchio 退回站控 `G(q)`。页面切「仿真臂」再切回真机仍不自动使能。无 conda pinocchio 时须显式 `-SkipPinocchio`（仅站控 `G(q)`）。便携包 `app/README.md` 必须是客户使用说明（源文件 `docs/客户使用说明.md`），不得再打包仓库根 `README.md` 那种开发说明。桌面快捷方式图标必须是 `packaging/windows/FAFUArmStation.ico`（FAFU Logo 圆角图标），由 `install_shortcut.vbs` 指向安装目录里的该文件，不得再用 Edge / shell32 占位图标。
