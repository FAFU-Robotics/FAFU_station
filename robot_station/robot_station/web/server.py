...
def serve_http(station: Station, cfg: StationConfig, stop_event: threading.Event) -> None:
    ...
    def dispatch(path: str, method: str, headers: Headers) -> Response | None:
        ...
        if path == "/api/info" and method == "GET":
            ...
            boot = str(snap.get("code_rev") or "").strip()
            arm_snap = snap.get("arm") or {}
            return _json(
                200,
                {
                    "ok": True,
                    ...
                    "code_rev": boot,
                    "boot_rev": boot,
                    "dyn_ready": bool(arm_snap.get("dyn_ready")),
                },
            )
        ...
        if path.split("?", 1)[0] == "/api/urdf" and method == "GET":
            from robot_station.grav_urdf import list_gravity_urdfs, selected_urdf_name, urdf_dir
        ...
        if path.split("?", 1)[0] == "/api/traj" and method == "PATCH":
            from urllib.parse import parse_qs
            ...
            qs = parse_qs(urlsplit(path).query)
            name = str((qs.get("name") or qs.get("file") or [""])[0] or "")
            dest = str((qs.get("to") or qs.get("dest") or [""])[0] or "")
            try:
                new = rename_recording(name, dest)
            except FileNotFoundError as exc:
                return _json(404, {"ok": False, "error": str(exc)})
                ...
                return _json(400, {"ok": False, "error": f"重命名失败: {exc}"})
            return _json(200, {"ok": True, "name": new.name})
        return _http(404, b"not found", "text/plain; charset=utf-8")
...
async def _on_text(st: Station, cid: str, raw: str, ws: ServerConnection) -> None:
    ...
    if kind == "clear":
        ...
        err = await _rpc(st.on_grav_urdf, cid, str(msg.get("name") or msg.get("urdf") or ""))
        await ws.send(json.dumps({"t": "ack", "ok": err is None, "error": err, "op": "grav_urdf"}))
    elif kind == "teach":
        q = msg.get("q") or []
        ...
        await ws.send(json.dumps({"t": "ack", "ok": err is None, "error": err, "op": "rec_rename"}))
    elif kind == "replay":
        speed = msg.get("speed")
        err = await _rpc(
            st.on_replay,
