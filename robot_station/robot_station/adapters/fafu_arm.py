...
``allow_motion=True`` (commands). Gravity / Gra+Fri match SDK
``test_fafu_motion_interactive.teach_record``: ~100 Hz
``gravity_compensation_step`` (pure ``clip(G(q)+optional friction)``, no
K/B/I). A short software ``damping_kd`` (``τ -= kd·v``) kills entry
velocity, then fades to 0 so the loop stays pure ``G(q)``. Official
``apply_compensation_torque(damping_kd=)`` has no firmware effect.
Pinocchio uses that SDK step after the fade; otherwise station ``G(q)`` + MIT.
Impedance still uses ``start_gravity_compensation``. Fake keeps slider
teach via ``teleport``.
"""
...
# After brake the last servo_j target is stale if the arm sagged or was moved.
STALE_CMD_RAD = math.radians(8.0)
...
def _live_rx_age_ms(robot: Any) -> float | None:
    """Age of the last CAN RX in ms. ``None`` means the driver never saw a frame.

    Hightorque ``Stats.last_rx_age_ms`` is ``-1`` until the first RX. Do not
    coerce that to ``0`` — ``0`` looks like a fresh frame and would latch
    ``_station_had_rx``, then a later timeout looks like an unplug.
    """
    ht = getattr(robot, "_ht", None)
    if ht is None or not hasattr(ht, "get_stats"):
        return None
    try:
        raw = getattr(ht.get_stats(), "last_rx_age_ms", None)
        if raw is None:
            return None
        age = float(raw)
    except Exception:
...
def _cached_joint_present(robot: Any) -> bool:
    ht = getattr(robot, "_ht", None)
    if ht is None or not hasattr(ht, "get_cached_state"):
        return False
    ids = [int(m) for m in list(getattr(robot, "_joint_motor_ids", None) or getattr(robot, "joint_motor_ids", []) or [])]
    if not ids:
        ids = list(range(1, 7))
    for mid in ids:
        try:
            if ht.get_cached_state(int(mid)) is not None:
                return True
        except Exception:
            continue
    return False
...
def _cached_motor_map(robot: Any, ids: list[int]) -> dict[int, Any]:
    """Cache-only motor states. Never blocking-read (that starves the bus)."""
    ht = getattr(robot, "_ht", None)
    want = [int(m) for m in ids]
    if ht is not None and hasattr(ht, "get_cached_state"):
        out: dict[int, Any] = {}
        for mid in want:
            try:
                st = ht.get_cached_state(int(mid))
    ...
    allow = set(want)
    return {int(k): v for k, v in states.items() if int(k) in allow}


def _bus_had_rx(robot: Any) -> bool:
    """True only after this session actually saw CAN RX (or leftover cache)."""
    if bool(getattr(robot, "_station_had_rx", False)):
        return True
    ...
    if age is not None and age <= timeout:
        _mark_bus_rx(robot)
        return True
    return _cached_joint_present(robot)


def _mark_bus_rx(robot: Any) -> None:
    try:
        robot._station_had_rx = True
    except Exception:
        pass


def _prime_joint_states(
    robot: Any,
    ids: list[int],
    *,
    ...
    """Live-read joints after USB open so the table is not stuck on empty cache."""
    ht = getattr(robot, "_ht", None)
    got: dict[int, Any] = {}
    ...
    for _ in range(max(1, int(rounds))):
        for mid in ids:
            ...
            if key in got:
                continue
            st = _read_motor_or_none(ht, key, timeout_s)
            if st is not None:
                got[key] = st
        if got:
            break
    ...
    if got:
        _mark_bus_rx(robot)
        _restart_live_polling(robot, [int(m) for m in ids])
        logger.info("已探测到电机 %s，重开状态轮询", sorted(got))
    return got


def _hardware_link_up(robot: Any) -> bool:
    """True while the debug-board serial is up.

    Unplug: we *had* RX, then ``last_rx_age_ms`` went stale. Startup with an
    empty cache and a large age is "motors not talking yet", not unplug.
    """
    if robot is None:
        return False
    ...
    if age is not None and age <= timeout:
        ...
        return True
    if name == "dead":
        if _lift_false_dead(robot):
            return True
        if not _bus_had_rx(robot):
            return True
        return False
    if _bus_had_rx(robot) and age is not None and age > timeout:
        return False
    return True
...
def _station_stream_link_ok(robot: Any, orig: Any = None) -> bool:
    ...
    A silent gripper must not latch DEAD. Never-seen-RX after open must not
    latch DEAD either — that is startup, not mid-stream power loss.
    """
    ht = getattr(robot, "_ht", None)
    try:
    ...
    if callable(enter):
        try:
            shown = float(age) if age is not None else timeout
            enter(
                f"no CAN RX for {shown:.0f} ms (motor power / USB / bus lost mid-stream?)"
            )
        except Exception:
            pass
    return False
...
def _read_motor_or_none(ht: Any, mid: int, timeout_s: float) -> Any:
    try:
        return ht.read_motor_state(int(mid), float(timeout_s))
    except Exception:
        return None


def _soft_precheck_communication(robot: Any) -> None:
    """Ping motors. Never raise: a silent joint must not fail Connect."""
    ht = getattr(robot, "_ht", None)
    cfg = getattr(robot, "_cfg", None)
    ...
    for mid in ids:
        if int(mid) in skip:
            bad.append(int(mid))
            continue
        checked.append(int(mid))
        if _read_motor_or_none(ht, int(mid), 0.5) is None:
            bad.append(int(mid))
    probed_bad = [m for m in bad if m not in skip]
    if checked and set(probed_bad) >= set(checked):
        time.sleep(0.2)
        still = [m for m in checked if _read_motor_or_none(ht, m, 1.0) is None]
        if still and len(still) < len(checked):
            bad = [m for m in skip] + still
        else:
            # All-silent at open is usually "bus not ready", not "arm gone".
            bad = [m for m in skip]
            logger.warning(
                "电机启动探测全部超时 %s，仍保持调试板连接，不把整臂标成未连接",
                checked,
            )
    robot._missing_motors = bad
    if bad:
        logger.warning(
            "电机 %s 未在探测窗口内应答，仍保持调试板连接；缺轴在电机表标离线",
            bad,
        )
    drop_absent_motors(robot, gripper_id=grip if grip is not None else None)
...
def _with_soft_precheck(controller_cls: type) -> type:
    ...
    if _uses_controlled_motors(controller_cls):
        # Overlay skips gripper, but its joint ping still raises. Always
        # replace _precheck_communication; leave overlay enable in place.
...
def _windows_short_path(path: str) -> str:
    ...
    try:
        ...
        n = int(get_short(path, None, 0) or 0)
        if n <= 0:
            return path
        buf = ctypes.create_unicode_buffer(n)
        if int(get_short(path, buf, n) or 0) <= 0:
            return path
...
def _ascii_cache_dir() -> Path:
    ...
    for raw in (
        ...
        try:
            ...
            last_err = exc
    raise OSError("找不到可写的 ASCII 目录，无法给官方 fafu_motor 准备 robot.cfg") from last_err


def native_openable_path(path: str | Path) -> str:
    """Path that MSVC ``ifstream`` / ``fopen`` can open on Windows.

    Official ``RobotConfig::load`` uses a narrow ``ifstream``. Python 3 passes
    UTF-8, which does not match the ANSI code page, so a repo under
    ``FAFU站控`` fails with 「无法打开配置文件」 even though the file exists.
    8.3 short names are often missing; copy to an ASCII cache when needed.
    """
    raw = str(path or "").strip()
    if not raw or raw.startswith("sim:"):
        return raw
    ...
    try:
        ...
        src = src.absolute()
    text = str(src)
    if _is_ascii_path(text):
        return text
    short = _windows_short_path(text)
    if short and _is_ascii_path(short) and Path(short).exists():
        return short
    if not src.is_file():
        return text
    dest = _ascii_cache_dir() / (src.name or "robot.cfg")
    try:
        need = (not dest.is_file()) or dest.stat().st_mtime < src.stat().st_mtime or dest.stat().st_size != src.stat().st_size
    except OSError:
        need = True
    if need:
        shutil.copy2(src, dest)
    logger.info("官方 fafu_motor 无法打开非 ASCII 配置路径，已改用 %s", dest)
    return str(dest)
...
class FafuArm(ArmAdapter):
    ...
    def __init__(
        ...
        *,
        ...
        self._traj_name = ""
        self._grav_urdf_name = ""
        self._grav_masses: tuple[float, ...] | None = None
        self._grav_coms: tuple[tuple[float, float, float], ...] | None = None
        self._grav_joints: tuple[tuple[tuple[float, float, float], tuple[float, float, float]], ...] | None = None
        self._load_grav_urdf()
        ...
        self._reprobe_th: threading.Thread | None = None
    ...
    def start(self) -> None:
        ...
        try:
            ...
            native_cfg = native_openable_path(cfg_path) if not sim else cfg_path
            kwargs: dict[str, Any] = {
                "cfg_path": native_cfg,
                "has_gripper": self.has_gripper,
                "auto_enable": False,
            ...
            logger.info(
                ...
                native_cfg if native_cfg != cfg_path else cfg_path,
                self.port or "cfg/auto",
            )
            robot = _with_soft_precheck(controller_cls)(**kwargs)
            ...
            primed: dict[int, Any] = {}
            if not _cached_joint_present(robot):
                primed = _prime_joint_states(
                    robot, list(self._full_joint_ids), timeout_s=0.3, rounds=2
                )
            if primed:
                logger.info("连接后补探测到电机 %s", sorted(primed))
            ...
            # Always poll the configured joints so a late-waking axis can appear.
            # Do not shrink the poll set to the first IDs that answered.
            poll_ids = list(self._full_joint_ids)
            if poll_ids and getattr(robot, "_ht", None) is not None:
                _restart_live_polling(robot, poll_ids)
            ...
            self._start_reprobe(robot, list(self._full_joint_ids))
    ...
    def _load_grav_urdf(self, name: str | None = None) -> str | None:
        try:
            dyn = load_selected_dynamics(name)
        except (FileNotFoundError, ValueError, OSError) as exc:
            if name:
                return str(exc)
            self._grav_urdf_name = selected_urdf_name()
            self._grav_masses = None
            self._grav_coms = None
            self._grav_joints = None
            return None
        if dyn is None:
            self._grav_urdf_name = ""
            self._grav_masses = None
            ...
            self._grav_joints = None
            return None
        self._grav_urdf_name = dyn.name
        self._grav_masses = dyn.masses
        self._grav_coms = dyn.coms
        self._grav_joints = dyn.joints
        return None

    def _gravity_urdf_path(self) -> Path | None:
        if self._grav_urdf_name:
            try:
                return resolve_gravity_urdf(self._grav_urdf_name)
            except FileNotFoundError:
                pass
        return default_urdf_path(self._sdk_root)

    def _urdf_snap(self) -> dict[str, Any]:
        return {
    ...
    def set_grav_urdf(self, name: str) -> str | None:
        ...
        try:
            ...
            return str(exc)
        err = self._load_grav_urdf(path.name)
        if err:
            return err
        persist_selected_urdf(path.name)
        robot = self._robot
        if robot is not None:
            self._setup_dynamics(robot, sim=self._is_fake())
        if self._use_sdk_float() and (
            self._float_writer_active() or self._ctrl_mode in FLOAT_MODES
        ):
            friction = self._ctrl_mode != "Gravity"
            impedance = self._ctrl_mode == "Impedance"
            if not self._stop_gravity(join=True):
                return "URDF 已记下，但当前重力环未退出；下次进入时生效"
            return self._start_gravity_loop(friction=friction, impedance=impedance)
        return None

    def _teach_friction_arg(self, robot: Any) -> Any:
        """Same Fc/Fv as SDK ``test_fafu_motion_interactive`` teach."""
        try:
            import numpy as np

            mod = sys.modules.get(type(robot).__module__)
            cls = getattr(mod, "FrictionParams", None) if mod is not None else None
            if cls is None:
                return None
            return cls(
                fc=np.array(FRICTION_FC, dtype=float),
                fv=np.array(FRICTION_FV, dtype=float),
                vel_threshold=FRICTION_VEL_EPS,
            )
        except Exception:
            return None
    ...
    def _setup_dynamics(self, robot: Any, *, sim: bool) -> None:
        ...
        try:
            ...
            friction = self._teach_friction_arg(robot)
            if friction is not None:
                kwargs["friction"] = friction
            setup(**kwargs)
            if friction is not None:
                try:
                    robot._friction_params = friction
                except Exception:
                    pass
    ...
    def _stop_reprobe(self) -> None:
        stop = getattr(self, "_reprobe_stop", None)
        if stop is not None:
            stop.set()
        th = getattr(self, "_reprobe_th", None)
        self._reprobe_th = None
        if th is not None and th.is_alive() and th is not threading.current_thread():
            th.join(timeout=0.4)

    def _start_reprobe(self, robot: Any, ids: list[int]) -> None:
        """Keep pinging silent joints after Connect so a waking bus can fill the table."""
        self._stop_reprobe()
        if self._is_fake() or getattr(robot, "_ht", None) is None or not ids:
            return
        self._reprobe_stop = threading.Event()
        stop = self._reprobe_stop

        def _loop() -> None:
            while not stop.wait(0.75):
                if self._robot is not robot:
                    return
                if not _hardware_link_up(robot):
                    return
                ht = getattr(robot, "_ht", None)
                if ht is None or not hasattr(ht, "get_cached_state"):
                    return
                ...
                if not missing:
                    _mark_bus_rx(robot)
                    continue
                got = _prime_joint_states(robot, missing, timeout_s=0.35, rounds=1)
                if not got:
                    continue
                still = [m for m in (getattr(robot, "_missing_motors", None) or []) if int(m) not in got]
                try:
                    robot._missing_motors = still
                except Exception:
                    pass
                _restart_live_polling(robot, list(ids))

        self._reprobe_th = threading.Thread(target=_loop, name="fafu-reprobe", daemon=True)
        self._reprobe_th.start()
    ...
    def _servo_cmd_is_current(self) -> bool:
        """True when the last ``servo_j`` target still matches the measured pose."""
        if self._servo_cmd_rad is None or self._robot is None:
            return False
        try:
            meas = self._read_joint_q_rad(self._robot)
        except Exception:
            return False
        return self._near_q(self._servo_cmd_rad, meas, tol=STALE_CMD_RAD)

    def _adopt_measured_cmd(self) -> list[float] | None:
        """Forget the last servo target and resume from the measured pose.

        Brake has no holding torque, so the arm may sag after ESTOP. Home /
        replay must not ``servo_j`` the pre-estop command first.
        """
        robot = self._robot
        ...
        if robot is not None:
            try:
                ...
                q = None
        if not q:
            self._servo_cmd_rad = None
            return None
        self._servo_cmd_rad = list(q)
        self._stream_q_rad = list(q)
        self._target_deg = [math.degrees(x) for x in q[: self.n]]
        return self._servo_cmd_rad
    ...
    def _start_joint_path(self, waypoints_rad: list[list[float]], speed_deg_s: float) -> None:
        ...
        self._freeze_needed = False
        self._servo_intent = True
        # In a live servo session keep the current command so the first tick
        # interpolates. After brake / a large measured gap, start from now.
        if not self._servoing or not self._servo_cmd_is_current():
            self._adopt_measured_cmd()
    ...
    def rename_traj(self, path: str, dest: str) -> str | None:
        ...
        if self._replay_frames:
            return "回放中请先停止再重命名"
        try:
            old = resolve_traj_path(path)
            new = rename_recording(path, dest)
        except FileNotFoundError as exc:
            return str(exc)
            ...
            return f"重命名失败: {exc}"
        if self._traj_name == old.name:
            self._traj_name = new.name
        return None
    ...
    def _apply_brake(self) -> None:
        """SDK ``brake()`` / MODE_BRAKE 0x0F. Not MODE_STOP and not position hold."""
        if self._brake_applied:
            return
        robot = self._robot
        ...
        if self._servoing or self._sdk_servoing(robot):
            try:
                robot.servo_end("brake")
                ended = True
            except Exception:
                logger.exception("servo_end(brake) 失败")
            self._servoing = False
            self._clear_joint_watchdogs(robot)
            self._servo_idle.set()
        if not ended:
    ...
    def _send_zero_vel_hold(self, q_rad: list[float] | None = None) -> None:
        ...
        try:
            ...
            self._servo_cmd_rad = list(q_rad[: self.n])
            self._stream_q_rad = list(self._servo_cmd_rad)
    ...
    def _run_teach_gravity(self, robot: Any, *, friction: bool) -> None:
        """Same float loop as SDK ``test_fafu_motion_interactive.teach_record``.

        ``gravity_compensation_step`` when pinocchio loaded; otherwise
        ``clip(G(q)+friction)`` + ``apply_compensation_torque``. No K/B/I.
        Entry uses software ``damping_kd`` (``τ -= kd·v``) then fades to 0.
        """
        send = getattr(robot, "apply_compensation_torque", None)
        # Fake implements missing names via __getattr__ as callable stubs.
        # Only treat methods declared on the class as real SDK dynamics.
        step = (
            getattr(robot, "gravity_compensation_step", None)
            if callable(getattr(type(robot), "gravity_compensation_step", None))
            else None
        )
        compute = (
            getattr(robot, "compute_compensation_torque", None)
            if callable(getattr(type(robot), "compute_compensation_torque", None))
            else None
        )
        use_step = bool(self._dyn_ready) and callable(step)
        use_sdk_tau = bool(self._dyn_ready) and callable(compute) and callable(send)
        if not use_step and not callable(send):
            raise RuntimeError("控制器没有 apply_compensation_torque，无法下发重力力矩")
        self._prepare_torque_send(robot)
        friction_arg = self._teach_friction_arg(robot)
        if friction_arg is not None:
            try:
                robot._friction_params = friction_arg
            except Exception:
                pass
        n = self.n
        period = 1.0 / TEACH_RATE_HZ
        raw_lim = getattr(robot, "_dyn_tau_limit", None)
        # SDK stores a numpy vector; ``or`` on that array raises ValueError.
        lim = FAFU_TAU_LIMIT_NM if raw_lim is None else raw_lim
        t0 = time.monotonic()

        def _send(tau_nm: list[float], damp: float) -> None:
            try:
                send(tau_nm, damping_kd=damp)
            except TypeError:
                send(tau_nm)
            ...
            while not self._grav_stop.is_set():
                ...
                try:
                    try:
                        ...
                        raw_v = [0.0] * n
                    max_v = max((abs(float(x)) for x in raw_v), default=0.0)
                    kd = startup_damping_kd(tick - t0, max_v)

                    if use_step and kd <= 1e-9:
                        step(friction=friction)
                    elif use_sdk_tau and kd > 1e-9:
                        q = list(robot.get_joint_values(prefer_cache=True))
                        v = list(robot.get_joint_velocities(prefer_cache=True))
                        tau = apply_vel_damping(
                            list(compute(q, v, friction=friction)), v, kd
                        )
                        _send(clip_tau(tau, lim), kd)
                    elif use_step:
                        step(friction=friction, damping_kd=kd)
                    elif use_sdk_tau:
                        q = list(robot.get_joint_values(prefer_cache=True))
                        v = list(robot.get_joint_velocities(prefer_cache=True))
                        send(compute(q, v, friction=friction))
                    else:
                        ...
                        while len(v) < n:
                            v.append(0.0)
                        tau = compensation_torque(
                            q,
                            v,
                            friction=friction,
                            tau_limit=FAFU_TAU_LIMIT_NM,
                            masses=self._grav_masses,
                            coms=self._grav_coms,
                            joints=self._grav_joints,
                        )
                        tau = clip_tau(apply_vel_damping(tau, v, kd), FAFU_TAU_LIMIT_NM)
                        _send(self._vec_to_sdk(tau, robot), kd)
                except Exception as exc:
    ...
    def _run_station_gravity(self, robot: Any, *, friction: bool, impedance: bool) -> None:
        ...
        Impedance only: G(q) + friction + K/B/I, hold the entry pose.
        Gravity / Gra+Fri use ``_run_teach_gravity``.
        """
        send = getattr(robot, "apply_compensation_torque", None)
        ...
        k_soft = list(IMPEDANCE_K[:n])
        b_soft = list(IMPEDANCE_B[:n])
        i_soft = list(IMPEDANCE_I[:n])
        while len(k_soft) < n:
            k_soft.append(0.0)
            b_soft.append(0.0)
            i_soft.append(0.0)
        ...
        try:
            while not self._grav_stop.is_set():
                ...
                hold = lead_through_step(
                    ...
                    fast_time,
                    slow_time,
                    hold_on_release=hold_on_release,
                    move_vel_thresh=MOVE_VEL_THRESH,
                    enter_time=DRAG_ENTER_S,
                    settle_time=DRAG_SETTLE_S,
                    rest_thresh=REST_VEL_THRESH,
                )
                tau = compensation_torque(
                    q,
                    v,
                ...
                for i in range(n):
                    ...
                    if dt > 0.0 and i_soft[i] > 0.0:
                        if hold[i]:
                            ...
                            integ[i] *= 0.95
                        cap = I_CLAMP_NM / max(i_soft[i], 1e-9)
    ...
    def _start_gravity_loop(self, *, friction: bool, impedance: bool) -> str | None:
        ...
        def run_teach() -> None:
            try:
                self._run_teach_gravity(robot, friction=friction)
            except Exception as exc:
                self._grav_err = str(exc)
                logger.exception("示教重力补偿环退出")

        def run_sdk_impedance() -> None:
            kwargs: dict[str, Any] = {
                "friction": friction,
                "rate_hz": 200.0,
            ...
            try:
                ...
                self._grav_err = str(exc)
                logger.exception("阻抗补偿环退出")

        def run_station_impedance() -> None:
            try:
                self._run_station_gravity(robot, friction=friction, impedance=True)
            except Exception as exc:
                self._grav_err = str(exc)
                logger.exception("站控阻抗补偿环退出")

        if impedance:
            target = run_sdk_impedance if use_sdk else run_station_impedance
            name = "fafu-impedance" if use_sdk else "station-impedance"
            if not use_sdk:
                logger.info("阻抗环使用站控 G(q)+K/B/I（未加载 pinocchio）")
        else:
            target = run_teach
            name = "fafu-teach-gravity" if use_sdk else "station-teach-gravity"
            logger.info(
                "重力环按 SDK 示教：gravity_compensation_step / G(q) 100Hz friction=%s pinocchio=%s",
                friction,
                use_sdk,
            )
    ...
    def _recover_dead_gravity(self) -> None:
        ...
        if self._brake_latched:
            try:
                ...
                logger.exception("重力环异常退出后急停刹车失败")
    ...
    def home(self, speed_deg_s: float) -> str | None:
        ...
        try:
            q_meas = self._read_joint_q_rad(robot)
        except Exception as exc:
            return f"读关节失败: {exc}"
        if self._servoing and self._servo_cmd_is_current():
            q_now = [math.degrees(x) for x in self._servo_cmd_rad or q_meas]
        else:
            self._servo_cmd_rad = list(q_meas)
            self._stream_q_rad = list(q_meas)
            q_now = [math.degrees(x) for x in q_meas]
    ...
    def emergency_stop(self) -> None:
        """Engage SDK brake (MODE_BRAKE). Do not call SDK emergency_stop (MODE_STOP / free-spin)."""
        fresh = not self._brake_latched
    ...
    def poll(self) -> ArmSnap:
        ...
        try:
            ...
            lookup = list(ids)
            if self.has_gripper:
                lookup.append(int(self.gripper_id))
            live_ht = not self._is_fake() and getattr(robot, "_ht", None) is not None
            states: dict = {}
            if live_ht:
                # Cache only. SDK get_motor_states(prefer_cache=True) falls back
                # to a 100 ms blocking read per missing id and starves polling.
                states = _cached_motor_map(robot, lookup)
            else:
                try:
            ...
            try:
                if live_ht:
                    moving = any(
                        abs(float(getattr(states.get(int(mid)), "velocity", 0) or 0)) > 0.02
                        for mid in ids
                        if states.get(int(mid)) is not None
                    )
                else:
