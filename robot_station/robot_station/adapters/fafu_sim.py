...
class FakeFafuController:
    ...
    def __init__(self, cfg_path: str, **kwargs: Any) -> None:
        ...
        self.last_damping_kd = 0.0
        self.last_grav_kwargs: dict[str, Any] = {}
    ...
    def setup_dynamics(self, *args: Any, **kwargs: Any) -> None:
        ...
        self.dyn_ready = True
        path = kwargs.get("urdf_path")
        if path is None and args:
            path = args[0]
        self.urdf_path = str(path) if path else ""
    ...
    def servo_end(self, finish_mode: str = "hold") -> None:
        ...
        self.is_servoing = False
        if finish_mode == "brake":
            self.brake()
        elif finish_mode == "hold":
            self.hold_position()
    ...
    def gravity_compensation_step(
        ...
        **kwargs: Any,
        ...
        self._require_motion("gravity_compensation_step")
        self.last_grav_kwargs = {"friction": friction, "damping_kd": damping_kd, **kwargs}
        # Ready-pose shoulder/elbow hold, same order of magnitude as G(q).
        tau = [0.0, 2.4, 1.6, 0.4, 0.12, 0.06]
        gain = float(damping_kd)
        if gain > 1e-9:
            with self._mlock:
                vel = list(self.v_rad_s)
            tau = [t - gain * float(vel[i] if i < len(vel) else 0.0) for i, t in enumerate(tau)]
        self.apply_compensation_torque(tau, damping_kd=gain)
        return list(self.last_tau_nm)

    def start_gravity_compensation(self, *args: Any, **kwargs: Any) -> None:
        self._require_motion("start_gravity_compensation")
        self.last_grav_kwargs = dict(kwargs)
        if not self.is_enabled:
            raise RuntimeError("仿真未使能")
    ...
    def apply_compensation_torque(self, tau: Iterable[float], *, damping_kd: float = 0.0) -> None:
        ...
        self.last_damping_kd = float(damping_kd)
    ...
    def step(self, dt: float) -> None:
        ...
        with self._mlock:
            ...
            if not self.allow_motion or self._estopped or self._braked or not self.is_enabled:
                self.v_rad_s = [0.0] * 6
                return
