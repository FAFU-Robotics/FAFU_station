...
class Station:
    ...
    def on_grav_urdf(self, cid: str, name: str) -> str | None:
        return self.core.on_grav_urdf(cid, name)
    ...
    def on_rec_rename(self, cid: str, path: str, dest: str) -> str | None:
        return self.core.on_rec_rename(cid, path, dest)
...
class WebFront:
    ...
    def on_clear_estop(self, cid: str) -> str | None:
        ack = self.motion.call({"t": "clear", "cid": cid}, timeout_s=20.0)
        if ack.get("ok"):
            return None
    ...
    def on_home(self, cid: str, speed: float) -> str | None:
        ack = self.motion.call({"t": "home", "cid": cid, "speed": speed}, timeout_s=20.0)
        if ack.get("ok"):
            return None
    ...
    def on_grav_urdf(self, cid: str, name: str) -> str | None:
        ack = self.motion.call({"t": "grav_urdf", "cid": cid, "name": name})
        if ack.get("ok"):
            return None
        return str(ack.get("error") or "切换 URDF 失败")
    ...
    def on_rec_rename(self, cid: str, path: str, dest: str) -> str | None:
        ack = self.motion.call({"t": "rec_rename", "cid": cid, "file": path, "name": dest})
        if ack.get("ok"):
            return None
        return str(ack.get("error") or "重命名失败")
