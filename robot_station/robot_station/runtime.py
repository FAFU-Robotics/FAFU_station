...
CODE_REV_FILES = (
    ...
    "robot_station/adapters/fafu_dyn.py",
    "robot_station/vendor_fafu/fafu_robot_controller.py",
    "robot_station/motion.py",
    "robot_station/grav_urdf.py",
    "robot_station/web/server.py",
    "robot_station/service.py",
    "robot_station/lock.py",
)

_BOOT_REV: str | None = None


def station_code_rev(root: Path | None = None) -> str:
    """Short content hash of the live-arm control path on disk now."""
...
def freeze_boot_code_rev(root: Path | None = None) -> str:
    """Pin the motion-process hash at startup. Do not recompute from later edits."""
    global _BOOT_REV
    _BOOT_REV = station_code_rev(root)
    return _BOOT_REV


def boot_code_rev() -> str:
    if _BOOT_REV is None:
        return freeze_boot_code_rev()
    return _BOOT_REV
