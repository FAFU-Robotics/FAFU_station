...
3. 本机要有 **conda-forge pinocchio + Python 3.10**（`conda create -n fafu-station -c conda-forge python=3.10 pinocchio`，或已有 `panthera`）。打包脚本拷进私有运行时；Windows pip 装不了真 Pinocchio。可用 `STATION_PYTHON` / `STATION_PINOCCHIO` 指定解释器。
...
| `runtime\python310\` | 私有 CPython 3.10 + 本软件依赖 + 打包机拷入的 pinocchio |
| `app\` | 站控源码 |
...
第一次会装到 `%LOCALAPPDATA%\FAFUArmStation`（不写 PATH）并打开页面；USB 连上后自动 Connect 并使能。以后点桌面快捷方式即可。操作说明见包内 `app\README.md`。

更新：再发一份新 zip，双击其中的 exe，会覆盖用户目录里的副本。

## 隔离边界

- **关在安装目录里：** Python 3.10、websockets / PyYAML / pywebview / numpy / pyrealsense2、conda-forge pinocchio（及 `Library\bin` DLL）、站控代码、SDK
