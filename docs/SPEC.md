...
- Connection：Connect / Disconnect（`start` / `stop`）。启动真机（`arm=fafu` 且允许运动）在串口连上后自动 `enable`（SDK 构造仍 `auto_enable=False`）。**一颗电机（含夹爪）没起来不得把整臂标成未连接**：调试板串口连上即为 Link 已连接；缺轴只在电机表标离线，状态栏写「已连接 · n/m 轴在线」。SDK `_precheck_communication` 在站控里改为警告而非整段失败。`get_joint_values` 缺一轴反馈时保持连接并用已有反馈/上次角度。**交付缺轴（夹爪或任意电机未装/未上电）时，使能与控制只针对在线电机，不得因缺轴拒绝整臂。** 官方 `enable()` / `is_enabled()` / `servo_start` 仍按全部 `cfg.motor_ids` 工作；站控用 Python 配置代理摘掉无应答 ID，使能与 `servo_start` 只切换在线关节（禁止再走 SDK 全员 `enable()` / `motor_reset`）。不得把 SDK `_has_gripper=False` 而 M7 仍留在 `motor_ids` 里（否则 `servo_j` 每拍 hold 离线夹爪）。夹爪离线只在电机表/顶栏提示，不得写成红色 `link_err` 盖住运动错误。未勾选「实时跟随」时拖滑条不下发，须点「发送位置」或勾选跟随。**臂源**：`t: arm_src` `mode=sim|fafu` 热切换适配器（先 hold 再关旧臂）。仿真不开 USB；切回真机不自动 `enable`。无 `STATION_ALLOW_LIVE_ARM=1` 时拒绝切真机。`hold()` 冻结当前角，不得把位置保持误做成 `MODE_STOP` 下坠。**急停保持当前姿态**（位置环 `hold` / `MODE_POSITION`），禁止调用 SDK `emergency_stop()`（那是 PWM 关闭、关节自由落体）。急停锁存后其它运动拒绝；**「复位」例外**：可从保持姿态受控回零。`DEAD` 需 `recover` 再 `resume`。
